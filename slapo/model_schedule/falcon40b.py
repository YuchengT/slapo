...
"""HuggingFace tiiuae/falcon-40b with model schedule."""
...
def _apply_schedule(
    ...
    **sch_config,
    ...
    logger.info(f"{sch.mod.parameters}", ranks=[0])
...
def replace_attention(
    ...
    init_config = dict(
        ...
        output_proj=True,
        attn_pdrop=model_config.hidden_dropout,
        resid_pdrop=model_config.hidden_dropout,
        attn_op_name=attn_op_name,
        fused_qkv=True,
    )
...
def replace_mlp(
    ...
    for idx in range(model_config.num_hidden_layers):
        ...
        sub_sch = sch[prefix]
        hidden_size, inter_size = sub_sch.mod.dense_h_to_4h.weight.shape
        with init_empty_weights(enable=delay_init):
            new_mod = FusedMLP(
                hidden_size,
                inter_size,
...
def shard_parameters(
    ...
    if "embed" in shard_target:
        word_embed_name, final_ln_name = "word_embeddings", "ln_f"
    ...
    if "mlp" in shard_target:
        ...
        for idx in range(model_config.num_hidden_layers):
            ...
            sub_sch[fc_names[1]].shard("weight", axis=0)
            ...
            if sequence_parallel:
                ...
                sub_sch[fc_names[1]].sync(mode="fwd_post", sync_op_or_fn="all_gather", axis=1)
